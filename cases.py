...
File        : cases.py
Description : Case Management - list view + new case form
Developer   : Member 2
"""
...
def _gen_case_number():
    ...
    rows = run_query(
        f"SELECT COUNT(*) FROM cases WHERE case_number LIKE 'CYB-{year}-%'"
    )
...
class CasesFrame(ctk.CTkScrollableFrame):
    ...
    def _build_list(self):
        ...
        # Filters - using pack side by side
        ...
        ctk.CTkOptionMenu(priority_col, values=["All"] + PRIORITIES,
    ...
    def _reload_list(self):
        ...
        conds = []
        if sf != "All":
            conds.append(f"c.status='{sf}'")
        if pf != "All":
            conds.append(f"c.priority='{pf}'")
        if conds:
        ...
        rows = run_query(sql)
        if not rows:
            ctk.CTkLabel(self.list_frame, text="No cases found.",
    ...
    def _case_card(self, parent, row):
        pri_colors = {"critical": RED, "high": ORANGE,
                      "medium": "#facc15", "low": GREEN}
        ...
        if role in ("admin", "investigator"):
            ...
            ctk.CTkOptionMenu(bot, values=STATUSES, variable=status_var,
                              width=200, fg_color=BG_INPUT,
                              button_color=PURPLE,
                              button_hover_color=PURPLE2,
            ...
            def _update(cid=case_id, sv=status_var):
                ns = sv.get()
                execute_command("UPDATE cases SET status=? WHERE case_id=?", (ns, cid))
                if ns == "closed":
                    execute_command(
                        "UPDATE cases SET closed_date=? WHERE case_id=?",
                        (str(datetime.now().date()), cid)
                    )
                self._reload_list()
    ...
    def _build_new(self):
        ...
        auto_cn = _gen_case_number()

        # Case Number (readonly)
        self._lbl(inner, "CASE NUMBER")
        ctk.CTkEntry(inner, textvariable=ctk.StringVar(value=auto_cn),
        ...
        # Row 1: type + priority  (pack side by side - NO grid)
        ...
        # Row 2: investigator + jurisdiction
        ...
        ctk.CTkButton(inner, text="Register Case", height=46,
                      ...
                      corner_radius=10,
                      command=lambda cn=auto_cn: self._submit(cn)
    ...
    def _submit(self, case_number):
        ...
        try:
            execute_command(
                "INSERT INTO cases (case_number,title,case_type,priority,"
                "lead_investigator_id,opened_date,status,affected_org,jurisdiction)"
                " VALUES (?,?,?,?,?,?,?,?,?)",
                (case_number, title, self.type_var.get(), self.priority_var2.get(),
                 self.inv_map[self.inv_var.get()], str(datetime.now().date()),
                 "open", org, self.jur_var.get())
            )
            self.msg_label.configure(
                text=f"Case {case_number} registered successfully.", text_color=GREEN)
