...
File        : database.py
Description : SQLite database connection, schema creation, and query helpers
...
def run_query(sql, params=()):
    """Run SELECT — returns list of sqlite3.Row objects."""
    conn = get_connection()
    try:
...
# ─────────────────────────────────────────────
#  Schema + Sample Data — runs once on startup
# ─────────────────────────────────────────────
...
def seed_sample_data(conn):
    """Insert sample data — called only if users table is empty."""
    cur = conn.cursor()
...
def init_db():
    """Create schema and seed data if database is fresh."""
    conn = get_connection()
